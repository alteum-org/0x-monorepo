...
blockchainTests.resets.only('DydxBridge unit tests', env => {
    ...
    describe('bridgeTransferFrom()', () => {
        ...
        const callBridgeTransferFrom = async (from: string, bridgeData: BridgeData, sender: string): Promise<string> => {
            const returnValue = await testContract.bridgeTransferFrom(
        ...
        const callBridgeTransferFromAndVerifyEvents = async (actionType: number, actionAddress: string, from: string, bridgeData: BridgeData, sender: string): Promise<void> => {
            // Execute transaction.
            const txReceipt = await testContract.bridgeTransferFrom(
                constants.NULL_ADDRESS,
                from,
                receiver,
                new BigNumber(1),
        ...
        it('reverts if called by an unauthorized account', async () => {
            const callBridgeTransferFromPromise = callBridgeTransferFrom(accountOwner, defaultBridgeData, notAuthorized);
